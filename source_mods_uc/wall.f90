...
CFACE_LOOP: DO ICF=INTERNAL_CFACE_CELLS_LB+1,INTERNAL_CFACE_CELLS_LB+N_INTERNAL_CFACE_CELLS
   ...
   CALL CFACE_THERMAL_GASVARS(ICF,SURF_INDEX,B1,T)
   RSUM_G  = CFA%RSUM_G
   MU_G  = CFA%MU_G
...
REAL(EB) :: ARO,QNET,RAMP_FACTOR,RHO_G_2,RSUM_F,PBAR_F,TMP_OTHER_SOLID,TSI,UN, &
            ...
            BBB,CCC,PPP,QQQ,RRR,UUU,YYY,WWW,HTC_OLD,RSC_LOC
...
# endif                 
    ...
    !!!!  !there's new code in newer version that is missing here
...
SELECT CASE(SIM_MODE)
   ...
   CASE (LES_MODE)
      ...
      DO N=1,N_TRACKED_SPECIES
         IF (SPECIES_MIXTURE(N)%SC_T_USER>TWO_EPSILON_EB) RSC_LOC=1._EB/SPECIES_MIXTURE(N)%SC_T_USER
         B1%RHO_D_F(N) = B1%RHO_F*( D_Z(ITMP,N) + (MU_G-MU_DNS(IIG,JJG,KKG))/B1%RHO_G*RSC_LOC )
      ENDDO
   CASE (DNS_MODE)
      ITMP = MIN(I_MAX_TEMP-1,NINT(B1%TMP_F))
      DO N=1,N_TRACKED_SPECIES
...
REAL(EB) :: RADIUS,AREA_SCALING,RVC,M_DOT_PPP_SINGLE,M_DOT_SINGLE,CP,MW_RATIO,H_G,&
            ZZ_GET(1:N_TRACKED_SPECIES),DENOM,M_GAS,LENGTH,WIDTH
REAL(EB), INTENT(IN) :: T,DT
INTEGER, INTENT(IN) :: NM
...
REAL(EB) :: RDT,DTMP,QDXKF,QDXKB,RR,RFACF,RFACB,RFACF2,RFACB2, &
            ...
            CP1,CP2,DENOM,SF_HTC_F,SF_HTC_B
