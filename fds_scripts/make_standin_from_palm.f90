...
!%%%%%% change for each run:
! Standalone:
INTEGER,PARAMETER:: IBAR=60, JBAR=60, KBAR=60, NT=901 !601
...
character(len=40)::run_name,end_file_name,in_file_name
...
! NTout=NT
 ...
 !
 ALLOCATE( T_ALL(1:(IBARout+2),1:(JBARout+2),1:(KBARout+2),1:NT )) 
 ALLOCATE( U_ALL(1:(IBARout+2),1:(JBARout+2),1:(KBARout+2),1:NT )) 
 ALLOCATE( V_ALL(1:(IBARout+2),1:(JBARout+2),1:(KBARout+2),1:NT )) 
 ALLOCATE( W_ALL(1:(IBARout+2),1:(JBARout+2),1:(KBARout+2),1:NT )) 
 ALLOCATE( H_ALL(1:(IBARout+2),1:(JBARout+2),1:(KBARout+2),1:NT )) 
 !
 T_ALL=0
 U_ALL=0
 V_ALL=0
...
t1=1     
  ...
  status=nf90_get_var(ncid_in, varid_t, T_ALL(1:(IBARout+2), 1:(JBARout+2), 1:(KBARout+2),1:NT) ,start=(/x1-1,y1-1,z1,t1/), count = (/ (IBARout+2), (JBARout+2), (KBARout+2) ,NT /))
  
  ! deal with level z=1 on PALM by not readin it 
  status=nf90_get_var(ncid_in, varid_w, W_ALL(1:(IBARout+2), 1:(JBARout+2), 2:(KBARout+2),1:NT ),start=(/x1-1,y1-1,z1,t1/), count = (/ (IBARout+2), (JBARout+2), (KBARout+1) ,NT /)) 
   
  ! 159:219 (in x) + 2 ghosts 
  status=nf90_get_var(ncid_in, varid_u, U_ALL(1:(IBARout+2), 1:(JBARout+2), 2:(KBARout+2),1:NT ),start=(/x1,  y1-1,z1,t1/), count = (/ (IBARout+2), (JBARout+2), (KBARout+1) ,NT /))
  
  ! 159:219 (in y) + 2 ghosts
  status=nf90_get_var(ncid_in, varid_v, V_ALL(1:(IBARout+2), 1:(JBARout+2), 2:(KBARout+2),1:NT ),start=(/x1-1,y1,  z1,t1/), count = (/ (IBARout+2), (JBARout+2), (KBARout+1) ,NT /))
...
!write file with variables for the internal grid only 
 ...
 status = nf90_def_var(ncid_out, "U", NF90_FLOAT, (/dimx,  dimy,  dimz,dimt /), varid_uout)
 status = nf90_def_var(ncid_out, "V", NF90_FLOAT, (/dimx,  dimy,  dimz,dimt /), varid_vout)
 status = nf90_def_var(ncid_out, "W", NF90_FLOAT, (/dimx,  dimy,  dimz,dimt /), varid_wout)
 ...
 status = nf90_put_var(ncid_out, varid_tout, t_all(2:(IBARout+1),2:(JBARout+1),2:(KBARout+1),1:NT))
 status = nf90_put_var(ncid_out, varid_uout, u_all(1:(IBARout+1),1:(JBARout+1),1:(KBARout+1),1:NT))
 status = nf90_put_var(ncid_out, varid_vout, v_all(1:(IBARout+1),1:(JBARout+1),1:(KBARout+1),1:NT))
 status = nf90_put_var(ncid_out, varid_wout, w_all(1:(IBARout+1),1:(JBARout+1),1:(KBARout+1),1:NT))
