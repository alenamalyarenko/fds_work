...
end_file_name= 'IC_from_' // TRIM(run_name)//  '.nc'
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
! IS THERE A FILE WITH WITH TIME STAMP? ONLY FOR timestep zero
 count=0
 TIME=1
  SECOND=TIME-1
  SKIPIT=.FALSE.  
     ...
     DO II2=0,I_UPPER
      ...
      DO I=1,ni
       ...
       DO J=1,nj
        ...
        DO K=1,nk 
         nnk=nnk+1
         t_all(GI1(NM)+nni,GJ1(NM)+nnj,GK1(NM)+nnk, count)=t_in(i,j,k)+273.15  !convert back to kelvins
...
!write file with variables for the internal grid only 
 ...
 status = nf90_put_var(ncid_out, varid_tout, t_all(2:61,2:61,2:61, 1))
 status = nf90_put_var(ncid_out, varid_uout, u_all(2:61,2:61,2:61, 1))
 status = nf90_put_var(ncid_out, varid_vout, v_all(2:61,2:61,2:61, 1))
 status = nf90_put_var(ncid_out, varid_wout, w_all(2:61,2:61,2:61, 1))
 status = nf90_put_var(ncid_out, varid_hout, h_all(2:61,2:61,2:61, 1))
