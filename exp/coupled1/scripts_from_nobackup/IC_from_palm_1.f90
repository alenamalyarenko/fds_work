...
nt=3600
  ...
  status=nf90_get_var(ncid_in, varid_u, u_in,start=(/x1,y1,z1,nt/), &
                                           count = (/ IBAR, JBAR, KBAR ,1 /))
  status=nf90_get_var(ncid_in, varid_v, v_in,start=(/x1,y1,z1,nt/), &
                                           count = (/ IBAR, JBAR, KBAR ,1 /))
  status=nf90_get_var(ncid_in, varid_w, w_in,start=(/x1,y1,z1,nt/), &
                                           count = (/ IBAR, JBAR, KBAR ,1 /)) 
  status=nf90_get_var(ncid_in, varid_t, t_in,start=(/x1,y1,z1,nt/), &
                                           count = (/ IBAR, JBAR, KBAR ,1 /))
...
!OUTPUT FDS FILES
       status= nf90_create('ic_palm.nc', NF90_CLOBBER, ncid_out)
