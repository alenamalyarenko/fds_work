...
workdir="/nesi/nobackup/scion03617/ama677/$path_to_file"
...
if [ "$mode" = "standalone" ]; then
    exec_src="/home/ama677/FDS/fds_work/backup_exec_hpc03/fds_impi_intel_linux_nc"
    fds_file="/home/ama677/FDS/fds_work/exp/standalone/${path_to_file}.fds"
elif [ "$mode" = "coupled" ]; then
    exec_src="/home/ama677/FDS/fds_work/backup_exec_hpc03/fds_impi_intel_linux_nc_coupled"
    fds_file="/home/ama677/FDS/fds_work/exp/coupled/${path_to_file}.fds"
else
...
cp "$exec_src" fds_impi_intel_linux_nc
...
if [ "$mode" = "coupled" ]; then
    ...
    if [ ! -f "/nesi/project/scion03617/ama677/fds_runs/input_data/$icfile" ]; then
        echo "Error: ICFILE '$icfile' not found in fds_scripts"
        exit 1
    fi
    if [ ! -f "/nesi/project/scion03617/ama677/fds_runs/input_data/$bcfile" ]; then
        echo "Error: BCFILE '$bcfile' not found in fds_scripts"
        exit 1
    fi

    cp "/nesi/project/scion03617/ama677/fds_runs/input_data/$icfile" ic_palm_c1.nc
    cp "/nesi/project/scion03617/ama677/fds_runs/input_data/$bcfile" bc_palm_c1.nc
fi

# --- Run FDS! ---
#mpirun -np 9 ./fds_impi_intel_linux_uc "$fds_file"
cp "/home/ama677/FDS/fds_work/hpc03_scripts/submit_${path_to_file}.bash" submit.bash
...
echo "Job $jid finished. Starting postprocessing..."


# --- Postprocessing step ---
cd /home/ama677/FDS/fds_work/fds_scripts
...
ifx -mcmodel=medium  -o program.exe  glue_output.f90 -L/$EBROOTNETCDFMINFORTRAN/lib/ -I/$EBROOTNETCDFMINFORTRAN/include -lnetcdf -lnetcdff

cp program.exe "$workdir"
cd "$workdir"
./program.exe "$runname" "$T_END"
mv "OUT_${runname}.nc" /nesi/project/scion03617/ama677/fds_runs/output_netcdf
