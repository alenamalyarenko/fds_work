...
#if defined coupled_bc
   IF (WC%BOUNDARY_TYPE==COUPLED_BOUNDARY)         CYCLE WALL_CELL_LOOP
#endif      
...
#if defined coupled_bc
   IF (IW<=N_EXTERNAL_WALL_CELLS .AND. BOUNDARY_TYPE/=OPEN_BOUNDARY .AND.BOUNDARY_TYPE/=COUPLED_BOUNDARY)   THEN
#else   
   IF (IW<=N_EXTERNAL_WALL_CELLS .AND. BOUNDARY_TYPE/=OPEN_BOUNDARY)   THEN 
#endif   
      RHOP(BC%II,BC%JJ,BC%KK) = 2._EB*B1%RHO_F - RHO_G 
   ENDIF
