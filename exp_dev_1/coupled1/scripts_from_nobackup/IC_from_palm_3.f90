...
!  status=nf90_get_var(ncid_in, dimxc, xc_in, start=(/158/),count=(/20/))
  ...
  !for velocity grids start is shifted by one to accomodate diff counting in fds                                         
  status=nf90_get_var(ncid_in, varid_u, u_in,start=(/x1+1,y1,z1,t1/), &
                                           count = (/ IBAR, JBAR, KBAR ,1 /))
  status=nf90_get_var(ncid_in, varid_v, v_in,start=(/x1+1,y1,z1,t1/), &
                                           count = (/ IBAR, JBAR, KBAR ,1 /))
